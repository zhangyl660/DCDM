...
from entropy_driven_guided_diffusion.image_datasets import load_data, load_data_specific, ForeverDataIterator, \
    load_domain_data
...
def main():
    ...
    model.load_state_dict(
        dist_util.load_state_dict(
            "pt/diffusion/256x256_classifier.pt", map_location=dist_util.dev()
        ),
        strict=False
    )
    logger.log('lode checkpoint complete')
    ...
    dataset = ConcatDataset([load_domain_data(
        data_dir=datadir,
        image_size=args.image_size,
        domain_id=domain_index,
        dataset_type='officeHome',
    ) for domain_index, datadir in enumerate(args.data_dirs)])
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        labels = extra["domain"].to(dist_util.dev())
